...
export interface MontantExtraction {
  ...
  article700: {
    /** % décisions du corpus qui mentionnent l'art. 700 ET un montant. */
...
/** Pour une décision, retourne les montants dans une fenêtre ±60 chars autour d'un pattern. */
function amountsNearPattern(
  ...
  for (const m of matches) {
    if (m.index === undefined) continue;
    const start = m.index - 60;
    const end = m.index + m[0].length + 60;
...
export function extractMontantsFromCorpus(
  ...
  return {
    ...
    article700: {
      tauxCondamnation:
        corpus.length > 0
          ? Math.round((art700DecisionCount / corpus.length) * 100)
          : null,
